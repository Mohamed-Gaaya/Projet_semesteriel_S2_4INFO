...
def home_view(request):
    ...
    published_jobs = Job.objects.filter().order_by('-timestamp')
    jobs = published_jobs.filter()
...
def job_list_View(request):
    ...
    """
    job_list = Job.objects.filter().order_by('-timestamp')
...
@login_required(login_url=reverse_lazy('login'))
@user_is_employer
def applicant_details_view(request, id,idjob):
    applicant = get_object_or_404(User, id=id)
    appl=Applicant.objects.get  (job=idjob,user=id)
    print(appl.pdf.url)
    ...
    response=render(request, 'jobapp/applicant-details.html', context)
